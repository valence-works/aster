...
The provider also implements `IResourceActivationStateReader`, which lets the core `IResourceVersionHistoryService` enumerate active channels for requested resource histories without exposing public SQL or changing schema. Single-resource and explicit batch version history inspection remain read-only and compose existing resource version, activation, and lifecycle marker rows.
