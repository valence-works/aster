# Implementation Plan: Query Validation Summaries

**Branch**: `030-query-validation-summaries` | **Date**: 2026-05-31 | **Spec**: [spec.md](spec.md)  
**Input**: Feature specification from `/specs/030-query-validation-summaries/spec.md`

## Summary

Add pure host-facing summary records and a `ToSummary()` helper for existing `QueryValidationResult` objects. The implementation will live beside the query validation models, aggregate validation failures deterministically by code, path, and feature, and preserve existing query validation/execution behavior. No provider changes, query planner, service registration, storage changes, public SQL, public `IQueryable<Resource>`, execution behavior changes, or new dependencies are introduced.

## Technical Context

**Language/Version**: C# latest, .NET 8.0 / 9.0 / 10.0 multi-targeting  
**Primary Dependencies**: Existing core SDK query validation models and xUnit test stack; no new dependencies  
**Storage**: No storage changes. Summaries are pure in-memory views over existing query validation result objects; no schema migration, persistence, provider storage, audit records, or physical index changes.  
**Testing**: `dotnet test Aster.sln`, focused query validation summary tests, existing query validator tests, `dotnet build Aster.sln /m:1`, `git diff --check`  
**Target Platform**: .NET library consumed by host applications  
**Project Type**: SDK/library  
**Performance Goals**: Linear in supplied failure count; no provider access, query execution, or planner work  
**Constraints**: Pure transformations only; deterministic ordinal ordering; ignore blank key-specific buckets; no mutation; no new runtime services  
**Scale/Scope**: One bounded reporting slice over query validation result objects
...
- **Provider agnostic**: PASS - Core-only pure aggregation over existing validation results.
- **Simplicity first**: PASS - Small records and one extension helper are the simplest current implementation.
...
- **Optimize for deletion**: PASS - One isolated model file and focused tests can be removed without affecting validation behavior.
...
```text
specs/030-query-validation-summaries/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
├── contracts/
│   └── query-validation-summaries.md
├── checklists/
│   └── requirements.md
└── tasks.md
```
...
└── Models/Querying/
    ...
    └── QueryValidationSummaries.cs

test/Aster.Tests/
└── Querying/
    ├── QueryValidationSummaryTests.cs
    └── ResourceQueryValidatorTests.cs
```

**Structure Decision**: Add a new summary model file under `Models/Querying` to keep validation result/failure contracts stable and avoid mixing aggregate helper code into core validation result records.
...
See [data-model.md](data-model.md), [contracts/query-validation-summaries.md](contracts/query-validation-summaries.md), and [quickstart.md](quickstart.md).
...
- **Explicitness over magic**: PASS - Explicit `ToSummary()` call only.
