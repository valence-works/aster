...
- **FR-008**: The Sqlite provider's store implementations (`IResourceDefinitionStore`, `IResourceWriteStore`, `IResourceQueryService`) MUST collectively support the full `IResourceManager` lifecycle contract when wired via DI: `CreateAsync` (singleton and duplicate-ID enforcement), `UpdateAsync` (optimistic version lock), `ActivateAsync` and `DeactivateAsync` (channel state management and `ChannelMode` enforcement), and all read operations (`GetAsync`, `GetVersionAsync`, `ListAsync`). Existing in-memory contract behaviour is the reference for expected outcomes.
