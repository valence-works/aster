...
### 3.1. Domain Models
*   **Definitions**: Must provide models for `ResourceDefinition`, `AspectDefinition`, and `FacetDefinition`. `ResourceDefinition` and `AspectDefinition` follow the **universal versioning pattern**: each carries a logical persistent identifier (`DefinitionId` / `AspectDefinitionId`) plus a version-specific `Id` and `Version` number. `FacetDefinition` is a simple field descriptor embedded within `AspectDefinition` — it carries only a `FacetDefinitionId`, `Type`, and `IsRequired` flag (no independent `Id` or `Version`).
