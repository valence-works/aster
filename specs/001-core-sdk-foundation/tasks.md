...
- [ ] T005 Create `ResourceDefinition` record in `src/core/Aster.Core/Models/Definitions/ResourceDefinition.cs` (Id, Version, AspectDefinitions, IsSingleton)
- [ ] T005a [P] Create `FacetDefinition` record in `src/core/Aster.Core/Models/Definitions/FacetDefinition.cs` (Id, Type, IsRequired) — typed sub-field within an Aspect (spec §3.1)
- [ ] T006 [P] Create `AspectDefinition` record in `src/core/Aster.Core/Models/Definitions/AspectDefinition.cs` (Id, RequiresName, Schema, FacetDefinitions)
- [ ] T007 Create `Resource` record in `src/core/Aster.Core/Models/Instances/Resource.cs` (Id, DefinitionId, Created, Owner)
- [ ] T007a [P] Create `AspectInstance` record in `src/core/Aster.Core/Models/Instances/AspectInstance.cs` (AspectDefinitionId, Name, Facets) — spec §3.1; unnamed key = `AspectDefinition.Id`, named key = `"{Id}:{Name}"` composite
- [ ] T007b [P] Create `FacetValue` record in `src/core/Aster.Core/Models/Instances/FacetValue.cs` (FacetId, Value) — spec §3.1; used in Query AST FacetValue filter expressions
- [ ] T008 Create `ResourceVersion` record in `src/core/Aster.Core/Models/Instances/ResourceVersion.cs` (ResourceId, Version, Created, Aspects, Hash) — immutable
...
- [ ] T011 Define custom exceptions: `VersionNotFoundException`, `ConcurrencyException`, `DuplicateAspectAttachmentException` in `src/core/Aster.Core/Exceptions/AsterExceptions.cs`
- [ ] T012 Transcribe `IResourceDefinitionStore` contract from `contracts/Abstractions.cs` into `src/core/Aster.Core/Abstractions/IResourceDefinitionStore.cs`
...
- [ ] T013a Define `IResourceWriteStore` contract (CreateResourceAsync, WriteVersionAsync, UpdateActivationAsync) in `src/core/Aster.Core/Abstractions/IResourceWriteStore.cs` — required by Constitution Principle V (Provider Agnostic)
...
- [ ] T018 [P] [US1] Write unit tests for `InMemoryResourceDefinitionStore` (register, get, list) in `test/Aster.Tests/InMemory/InMemoryResourceDefinitionStoreTests.cs`
...
- [ ] T019 Implement `InMemoryResourceStore` (private storage) using `ConcurrentDictionary<string, Resource>`, `ConcurrentDictionary<string, List<ResourceVersion>>`, and nested `ConcurrentDictionary` for channel activations in `src/core/Aster.Core/InMemory/InMemoryResourceStore.cs`
- [ ] T020 Implement `IResourceManager` as `InMemoryResourceManager` with `CreateAsync` (produces V1) and `UpdateAsync` (increments version, enforces `BaseVersion` ETag via `ConcurrencyException`) in `src/core/Aster.Core/InMemory/InMemoryResourceManager.cs`
...
- [ ] T024 [P] [US2] Write unit tests for create/update/versioning (version increment, immutability, optimistic lock) in `test/Aster.Tests/InMemory/InMemoryResourceManagerTests.cs`
...
- [ ] T026 [US3] Define `ITypedAspectBinder` interface (bind POCO to aspect key, serialize/deserialize) in `src/core/Aster.Core/Abstractions/ITypedAspectBinder.cs`
- [ ] T027 [US3] Implement `SystemTextJsonAspectBinder` using `System.Text.Json` with State Replace logic in `src/core/Aster.Core/Services/SystemTextJsonAspectBinder.cs`
- [ ] T028 [P] [US3] Add `GetAspect<T>` and `SetAspect<T>` extension methods on `ResourceVersion` in `src/core/Aster.Core/Extensions/ResourceVersionExtensions.cs`
- [ ] T029 [P] [US3] Add `WithTypedAspect<T>()` registration method to `ResourceDefinitionBuilder` in `src/core/Aster.Core/Definitions/ResourceDefinitionBuilder.cs`
- [ ] T030 [US3] Write unit tests for POCO round-trip (string, int, bool, decimal, date), State Replace semantics, and unknown-field handling in `test/Aster.Tests/Services/SystemTextJsonAspectBinderTests.cs`
...
- [ ] T036 [US5] Create `AsterCoreServiceCollectionExtensions.AddAsterCore()` in `src/core/Aster.Core/Extensions/AsterCoreServiceCollectionExtensions.cs` that registers `InMemoryResourceDefinitionStore`, `InMemoryResourceManager`, `InMemoryQueryService`, and `SystemTextJsonAspectBinder`
...
- [ ] T040 [US5] Add a minimal Razor/static HTML dashboard page to visualize definitions and instances in `src/apps/Aster.Web/Pages/Dashboard.cshtml` (or equivalent)
